...
def _handle_ConnectionUp (event):
      ...
      for m in event.connection.features.ports:
            ...
            if m.name == "s1-eth1":
                  ...
                  msg = of.ofp_flow_mod(match=of.ofp_match(dl_type=0x800,nw_src="10.0.0.1",nw_dst="10.0.0.2",nw_proto = pkt.ICMP_PROTOCOL),action=of.ofp_action_output(port=3))
                  event.connection.send(msg)

                  msg = of.ofp_flow_mod(match=of.ofp_match(dl_type=0x800,nw_src="10.0.0.2",nw_dst="10.0.0.1",nw_proto = pkt.ICMP_PROTOCOL),action=of.ofp_action_output(port=3))
                  event.connection.send(msg)
